...
const NavBarItem = (props) => {
    ...
    return (
        ...
        {navBarLeftItems.map((item)=>{
             return (  <li className={item.class}><NavLink className="nav-link  " to={item.to}>{item.name}</NavLink></li>)
        })}
        </ul>
         
         <ul className="nav navbar-nav navbar-right">
          {navBarRightItems.map((item)=>{
              return (  <li className="nav-item" ><NavLink to={item.to} className="nav-link"> <span className={item.class}></span> {item.name}</NavLink></li>)
            })}
